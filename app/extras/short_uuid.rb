...
class ShortUUID
  ...
  def self.generate(prefix=nil)
    #time   = (Time.now.to_f * 10_000_000).to_i   # nano-second level precision
    #time   = (Time.now.to_f * 1000).to_i         # milli-second
    time   = (Time.now.to_f * 10000).to_i         # 10th of a milli-second
    #time   = Time.now.to_i                       # second
    #jitter = rand(1000)                          # 1000 randomness factor
    #jitter = rand(10)                            # 10
    #key    = encode "#{jitter}#{time}".to_i, split: 4
    key    = encode time, split: 3                # NO RANDOMNESS !!!

    [prefix, key].compact.join('-')
  end

  def self.unique(prefix=nil)
    time   = (Time.now.to_f * 1_000_000_000).to_i # nano-second level precision
    jitter = rand(1000)                           # 1000 randomness factor
    key    = encode "#{jitter}#{time}".to_i, split: 5
